...
const Notification = ({ children, trigger }: DropdownMenuProps) => {
    ...
    return (
        <div className="relative inline-block text-left" ref={dropdownRef}>
            ...
            {isOpen && (
                <div
                    className="origin-top-right absolute right-0 mt-2 w-80 rounded-xl shadow-xl bg-white dark:bg-zinc-900 border focus:outline-none z-50 animate-in fade-in-0 zoom-in-95"
                    role="menu"
        ...
        <div className="flex items-start justify-between">
            ...
            <div className="flex items-center space-x-1 ml-2">
                {unread && onMarkRead && (
                    <button
                        onClick={onMarkRead}
                        className="p-1 text-zinc-400 cursor-pointer hover:text-green-600 transition-colors"
                        title="Mark as read"
                    >
                ...
                {onDismiss && (
                    <button
                        onClick={onDismiss}
                        className="p-1 text-zinc-400 cursor-pointer hover:text-red-600 transition-colors"
                        title="Dismiss"
                    >
                        <X className="w-4 h-4" />
                    </button>
...
export default function NotificationDropdown() {
    ...
    return (
        <div className="flex items-center justify-center font-sans">
            <Notification
                trigger={
                    <button className="relative p-2 text-zinc-600 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-100 cursor-pointer hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-lg transition-colors">
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        {unreadCount > 0 && (
                            <button
                                ...
                                className="text-sm cursor-pointer text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 font-medium"
                            >
                                Mark all read
                            </button>
                        )}
